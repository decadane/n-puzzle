...
public class Solver {
    ...
    private static class Stats {
        ...
        boolean sortBest;
        Map<Stat, Long> statistic = new HashMap<>(5);

        Stats(long cost, long expanded, long depth, long time, long memory) {
            sortBest = true;
            statistic.put(Stat.COST, cost);
            statistic.put(Stat.EXPANDED, expanded);
            statistic.put(Stat.DEPTH, depth);
            statistic.put(Stat.TIME, time);
            statistic.put(Stat.MEMORY, memory);
        }
        ...
        Stats(boolean sort) {
            ...
            if (sort == Solver.BEST) {
                // initial values high
                statistic.put(Stat.COST, Long.MAX_VALUE);
                statistic.put(Stat.EXPANDED, Long.MAX_VALUE);
                statistic.put(Stat.DEPTH, Long.MAX_VALUE);
                statistic.put(Stat.TIME, Long.MAX_VALUE);
                statistic.put(Stat.MEMORY, Long.MAX_VALUE);
            } else {
                // initial values low
                statistic.put(Stat.COST, -Long.MAX_VALUE);
                statistic.put(Stat.EXPANDED, -Long.MAX_VALUE);
        ...
        public void update(Stat which, long value) {
            Long prev = statistic.get(which);
            if (prev == null) return;
            if (sortBest) {
                if (value < prev.longValue())
                    statistic.put(which, value);
            } else {
                if (value > prev.longValue())
                    statistic.put(which, value);
            }
        ...
        public void update(Stats obs) {
            if (sortBest) {
                for (Stat s : Solver.Stat.values())
                    if (obs.recall(s) < statistic.get(s)) statistic.put(s, obs.recall(s));
            } else {
                for (Stat s : Solver.Stat.values())
                    if (obs.recall(s) > statistic.get(s)) statistic.put(s, obs.recall(s));
            }
        ...
        public long recall(Stat which) {
            Long value = statistic.get(which);
    ...
    public Solver(Board initial, Method method) {
        ...
        boardPQ = new PriorityQueue<>();
        boardDQ = new ArrayDeque<>();
//////////////////////////////////////////////////////////////////////////////////////
//  Use various packages for the hashset
        visited = new HashSet<>();  // default jdk
...
//////////////////////////////////////////////////////////////////////////////////////        
                ...
                for (Board b : fringe.board().neighbors())
                    if (!fringe.isRegression(b)) {
                        ...
                        if (method != Method.IDA ||
                            ...
                            if (method != Method.IDA && method != Method.ASD)
                                visited.add(ByteBuffer.wrap(b.getState()));
                            childrenAdded++;
                        }
                    }
    ...
    public int moves() {
        return this.isSolvable() ? fringe.moves() : -1;
    }
    ...
    public static void displayHelp() {
        ...
        System.out.println("Sliding Puzzle ru.kmedhurs.n_puzzle.Solver");
        System.out.println("Robert Wagner CISC 3410");
        System.out.println();
        System.out.println("USEAGE:");
        System.out.println("    java -jar ru.kmedhurs.n_puzzle.Solver.jar [options] [input filename]");
        System.out.println();
    ...
    private static void solve(Board initial, Method method) {
        ...
        if (Solver.outputSummary) {
            ...
            Solver.statistics.put(key, s);
                    /*
                    ru.kmedhurs.n_puzzle.Solver.best.update (Stat.COST,     solver.fringe.moves);
                    ru.kmedhurs.n_puzzle.Solver.worst.update(Stat.COST,     solver.fringe.moves);
                    ru.kmedhurs.n_puzzle.Solver.best.update (Stat.EXPANDED, solver.numExpanded);
                    ru.kmedhurs.n_puzzle.Solver.worst.update(Stat.EXPANDED, solver.numExpanded);
                    ru.kmedhurs.n_puzzle.Solver.best.update (Stat.DEPTH,    solver.maxDepth);
                    ru.kmedhurs.n_puzzle.Solver.worst.update(Stat.DEPTH,    solver.maxDepth);
                    ru.kmedhurs.n_puzzle.Solver.best.update (Stat.TIME,     solver.elapsedTime);
                    ru.kmedhurs.n_puzzle.Solver.worst.update(Stat.TIME,     solver.elapsedTime);
                    ru.kmedhurs.n_puzzle.Solver.best.update (Stat.MEMORY,   solver.maxMem);
                    ru.kmedhurs.n_puzzle.Solver.worst.update(Stat.MEMORY,   solver.maxMem);
                    */
            //ru.kmedhurs.n_puzzle.Solver.summary.add(solver.toSummary());
        }
    ...
    public static void main(String... args) {
        ...
        Set<Method> useMethods = new LinkedHashSet<>();
        Set<HeuristicType> useHeuristics = new LinkedHashSet<>();
                ...
                if (arg.charAt(0) == '-') {
                    if (arg.length() == 6 && arg.charAt(1) == '-') {
                        ...
                        switch (arg.substring(1).toUpperCase()) {
                            case "ALL":
                                useMethods.add(Method.BFS);
                                useMethods.add(Method.DFS);
                                useMethods.add(Method.ASD);
                                useMethods.add(Method.AST);
                                useMethods.add(Method.IDA);
                                useHeuristics.add(HeuristicType.HAM);
                                useHeuristics.add(HeuristicType.MAN);
                                useHeuristics.add(HeuristicType.INT);
                                break;
                            case "HELP":
                                displayHelp();
                                System.exit(0);
                            ...
                            case "IDA":
                                ...
                                break;
                            case "TOTAL":
                                Solver.outputTotal = true;
                                break;
                            case "SUMMARY":
                                Solver.outputSummary = true;
                                break;
                            case "HAM":
                                useHeuristics.add(HeuristicType.HAM);
                                break;
        ...
        if (useHeuristics.isEmpty()) useHeuristics.add(HeuristicType.INT);
        ...
        while (!finished) {
            ...
            Board initial = new Board(line, HeuristicType.INT);
            if (initial == null || !initial.isValid()) {
                if (showPrompt || !outputCSV)
                    System.out.println("  ru.kmedhurs.n_puzzle.Board is not valid, skipping.");
                continue;
            }
            if (!Solver.skipUnsolvable && !initial.isSolvable())
                if (showPrompt || !outputCSV) {
                    System.out.println("  ru.kmedhurs.n_puzzle.Board is not solvable, skipping.  use -noskip option to try anyway.");
                    continue;
                }
            if (outputSummary) {
                Solver.summary = new MultiBuffer();
                Solver.statistics = new HashMap<>();
            ...
            for (Method method : useMethods) {
                switch (method) {
                    ...
                    case DFS:
                        ...
                        break;
                    default:
                        for (HeuristicType heuristic : useHeuristics) {
